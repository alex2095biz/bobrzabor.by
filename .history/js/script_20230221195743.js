...
let scrollLeft = document.querySelectorAll(".fadeInSlide-left");
let scrollRight = document.querySelectorAll(".fadeInSlide-right");
...
let callbackR = function (entries, observer) {
  entries.forEach((entry) => {
    if (entry.isIntersecting) {
      entry.target.classList.add("fadeInSlide-right");
      observer.unobserve(entry.target);
    }
  });
};
let observerR = new IntersectionObserver(callbackR, options);

scrollRight.forEach((target) => {
  observerR.observe(target);
});
